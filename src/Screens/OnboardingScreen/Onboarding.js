...
const Onboarding = ({ navigation }) => {
    ...
    return (
        ...
        <ImageBackground style={{ width: '100%', height: Dimensions.get('window').height }} source={require('../../assets/Images/Background.png')} resizeMode='repeat'>
            <View style={{
                        ...
                        <ScrollView>
                            <View style={{ marginTop: Platform.OS == 'ios' ? StatusBar.currentHeight + 50 : 20, paddingHorizontal: 20 }}>
                                ...
                                <View style={{ marginVertical: 20 }}>
                                    <Text style={styles.headingText}>Choose your preferred language</Text>
                                ...
                                <FlatList data={language} renderItem={({ item, index }) => (
                                    <TouchableOpacity onPress={() => handleClick(item)}
                                        ...
                                        <View style={selectedLang?.name == item?.name ? [styles.iconContainer, { backgroundColor: '#FCB300' }] : styles.iconContainer}>
                                            <Icon name='check' size={14} color={selectedLang?.name == item?.name ? '#222222' : '#fff'} />
                                        </View>
                                    </TouchableOpacity>
                                )} />
                            </View>
                ...
                }
                {
                    showImage ? null :
                        <View style={{ position: 'absolute', bottom: 10, alignSelf: 'center' }}>
                            <ButtonComp color={true} text={'Select & Continue'} navigation={() => navigation.navigate(RouteTexts.BOTTOM_TABS)} />
                        </View>
                }
...
export const styles = StyleSheet.create({
    ...
    languageContainer: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 15 },
    iconContainer: { height: 20, width: 20, borderRadius: 10, borderWidth: 1, borderColor: '#fff', justifyContent: 'center', alignItems: 'center' }
})
