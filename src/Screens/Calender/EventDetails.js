...
const EventDetails = ({ navigation, route }) => {
    ...
    const keys = [
        { name: 'Start date', value: moment(item?.attributes?.start_date).format('ddd,MMMM DD , YYYY') },
        { name: 'End date', value: moment(item?.attributes?.end_date).format('ddd,MMMM DD , YYYY') },
    ...
    return (
        <View style={{ flex: 1, backgroundColor: '#ffffff' }}>
            ...
            <View style={styles.main}>
                ...
                <Text style={{ color: '#777777', fontFamily: 'Mulish-Regular', marginHorizontal: 10 }}>{item?.attributes?.category}</Text>
                ...
                <View style={{ paddingHorizontal: 10 }}>
                    ...
                    <FlatList bounces={false} contentContainerStyle={{ marginTop: 10 }} data={keys} renderItem={({ item, index }) => (
                        <View style={{ flexDirection: 'row', marginVertical: 5, justifyContent: 'space-between', width: '70%' }}>
                            ...
                            <View style={{ width: '80%', marginLeft: 20, justifyContent: 'center' }}>
                    ...
                    <FlatList
                        ...
                        renderItem={({ item, index }) => (
                            <Image
                                source={item?.attributes?.url ? { url: item?.attributes?.url } : require('../../assets/Images/Background.png')}
