...
const SpottingErrorPage = ({ setModalVisible, navigation }) => {
    ...
    const RenderItem = (item, index) => (
        <TouchableOpacity onPress={() => selectionHandler(item)} style={{ justifyContent: 'space-between', height: 40, alignItems: 'center', flexDirection: 'row' }}>
            <Text style={{ fontFamily: 'Mulish-Regular', color: '#222222' }}>{item?.name}</Text>
            <View style={{ justifyContent: 'center', alignItems: 'center', borderRadius: 10, width: 20, height: 20, borderColor: '#222222', borderWidth: 1 }}>
                <Icon name='check' size={14} color='#222222' />
            </View>
        </TouchableOpacity>
    )
    return (
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
            ...
            {
                showSubmitPage ? <SelectErrorPage /> :
