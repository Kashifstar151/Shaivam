...
const TextInputCom = ({ reg, headinText, insiderText, width, navigation, locationIcon, State, inputKey, value, height, returnType, multiLine }) => {
    console.log("🚀 ~ TextInputCom ~ State:", inputKey)
    const dispatch = useDispatch()
    const onChangeText = (e) => {
        // if (reg.matches(e)) {
        dispatch(setInputValue({ inputKey, inputValue: e }))
        // }
    }
    ...
    return (
        <View style={{ marginVertical: 10 }}>
            ...
            <View style={{ flexDirection: 'row', backgroundColor: '#F3F3F3', height: height ? height : 55, width: width ? width : Dimensions.get('window').width - 90, alignItems: 'center', paddingHorizontal: 20, borderRadius: 10, }}>
                ...
                <TextInput returnKeyType={returnType} value={value} onChangeText={(e) => onChangeText(e)} onFocus={navigation} style={{ paddingHorizontal: 10, height: height ? height : 55 }} placeholder={insiderText} multiline={multiLine} />
...
export const styles = StyleSheet.create({
    inputComp: { flexDirection: 'row', backgroundColor: '#F3F3F3', height: 55, width: Dimensions.get('window').width - 90, paddingHorizontal: 15, borderRadius: 10, }
})
