    ...
    const RenderImage = (item) => {
        ...
        return (
            <View style={{ marginHorizontal: 10, alignItems: 'center' }}>
                <TouchableOpacity style={{ zIndex: 20, position: 'relative', top: 10, left: 50, height: 20, width: 20, borderRadius: 10, justifyContent: 'center', alignItems: 'center', backgroundColor: '#C1554E' }} onPress={() => removeImage(item)}>
                    <Icon name='x' color='white' />
                </TouchableOpacity>
                <Image source={{ uri: item?.uri }} style={{ height: 100, width: 100, borderRadius: 10 }} />
                <Text style={{ fontSize: 12, fontFamily: 'Mulish-Regular' }}>{item?.fileName?.slice(-10)}</Text>
            </View>
        )
    }
    return (
                ...
                submitted ?
                    ...
                    : <View style={styles.mainContainer}>
                        ...
                        <View style={{ paddingHorizontal: 20 }}>
                            ...
                            <Text style={styles.descriptionText}>You can submit images for this temple</Text>
                        </View>
                        <View style={{ paddingHorizontal: 20, marginTop: 20 }}>
                            <Text style={[styles.descriptionText, { fontFamily: 'Mulish-Bold' }]}>Upload Images (You can upload multiple images)</Text>
                            <TouchableOpacity onPress={openGallary} style={styles.uploadContainer}>
                                <Icon name='image' size={24} color='#777777' />
                                <Text style={[styles.descriptionText, { fontSize: 14, marginHorizontal: 10 }]}>Click here to upload photo</Text>
                            </TouchableOpacity>
                        </View>
                        <View style={{ paddingHorizontal: 10 }}>
                            {
                                images?.length > 0 ?
                                    <FlatList contentContainerStyle={{ marginBottom: 20 }} horizontal data={images} renderItem={({ item, index }) => (
                                        RenderImage(item)
                                    )} /> :
